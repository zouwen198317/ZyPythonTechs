...
with tf.Session() as sess:
    ...
    for i in range(1001):
        ...
        # 记录训练集计算的参数
        summary = sess.run(merge, feed_dict={x: batch_xs, y: batch_ys, keep_prob: 1.0})
        train_writer.add_summary(summary, i)

        # 测试集数据
        batch_xs, batch_ys = minist.test.next_batch(batch_size)
        # 记录测试集计算的参数
        summary = sess.run(merge, feed_dict={x: batch_xs, y: batch_ys, keep_prob: 1.0})
        ...
        if i % 100 == 0:
            ...
            train_acc = sess.run(accuracy, feed_dict={x: minist.train.images[:10000], y: minist.train.labels[:10000],
                                                      keep_prob: 1.0})
